...
class URLutils():
    def __init__(self):
        ...
        self.slndrivers = [
            webdriver.Chrome(chrome_options=Options().add_argument("--headless"))
        ]
    ...
    def testAPI(self, url, api='ph'):
        if api=='ph':
            return self._photopea(url)
        elif api=='cb':
            return self._code_beautify(url)
        elif api=='pl':
            return self._pixlr(url)
